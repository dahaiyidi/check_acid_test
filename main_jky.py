...
def main():
    ...
    ls = ['姓名', '采样时间', '证件号码']
    ls_end = ['证件类型', '检测机构', '样本编码']
    data = pd.DataFrame(columns=ls + ['img'])  # 添加一列记录img name
    imgs = os.listdir(img_folder)

    ocr = PaddleOCR(lang='ch', det_db_box_thresh=0.5)  # need to run only once to download and load model into memory
    ...
    for img_path in tqdm(imgs):
        ...
        cur = []
        for col, col_end in zip(ls, ls_end):
            # col = '姓名'
            try:
        ...
        # 修正日期：日与时之间无空格
        if cur[1][10] != ' ':
            hour_start = cur[1].find(':') - 2
            cur[1] = cur[1][:hour_start] + ' ' + cur[1][hour_start:]
    ...
    for ind_img, row in data.iterrows():
        ...
        if has_id_info:
            # 匹配id前4位
            ind_id_prefix = all_students['证件号码'].str.startswith(id[:4])
            # 匹配id后4位
        ...
        # ind_id_prefix & ind_id_postfix： 证件号码前4位和后4位均匹配，且仅匹配到一位用户
        # (ind_id_prefix | ind_id_postfix) & ind_name_postfix 证件号码前4位或后4位均匹配， 且姓名匹配，且仅匹配到一位用户
        if has_id_info:
            matching_ls = [ind_id_postfix, ind_name_postfix, ind_id_prefix & ind_id_postfix, (ind_id_prefix | ind_id_postfix) & ind_name_postfix]
        else:
